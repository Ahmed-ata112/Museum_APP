              ...
              return Padding(
                ...
                child: LineChart(
                  ...
                  LineChartData(
                    ...
                    lineBarsData: [
                      LineChartBarData(
                        spots: [
                          ...
                          if(DateTime.parse(_goods.first!.time).year != now.year)  //i am comparing the first element only as i have ordered the query in descending order in the api python script
                            FlSpot(0,0),
                          if(DateTime.parse(_goods.first!.time).year == now.year)  //i put another if statement as when i used else it gives me an error
                            for(var s in _goods)
                              if(DateTime.parse(s.time).year == now.year)
                                FlSpot(DateTime.parse(s.time).month.toDouble(),s.quantity.toDouble())
                            //else
                              //FlSpot(0,0)
                    ...
                    axisTitleData: FlAxisTitleData(show: true, leftTitle: AxisTitle(showTitle: true, titleText: "quantity sold", textStyle:const TextStyle(color: Colors.black, fontSize: 14))
                    ,bottomTitle: AxisTitle(showTitle: true, titleText: "month (current year)", textStyle:const TextStyle(color: Colors.black, fontSize: 14))),
